...
public abstract class AbstractGame implements Game {
    ...
    private void auctionPlants() {
        ...
        // Keep track of which players have declined to put up a plant for
        // auction
                ...
                if (plant == null) {
                    if (mandatory) {
                        throw new IllegalStateException(
                                "Players must select a plant");
                    }
                    playersWhoHaveOptedOut.add(player);
                }
                else {
                    // Auction the selected plant
                    auction(plant, playersWhoHaveBoughtPlants,
                            playersWhoHaveOptedOut, player);
                }
            }
        }
    ...
    /**
     * Auctions off the given plant to the players who have not yet bought a
     * plant or opted out of the auction phase
     * 
     ...
     * @param playersWhoHaveBoughtPlants players who have bought plants this
     *            turn; can't be <code>null</code>
     * @param firstBidder the player to bid first
     */
    private void auction(final Plant plant,
            final Set<Player> playersWhoHaveBoughtPlants,
            final Set<Player> playersWhoHaveOptedOut, final Player firstBidder) {
        Utils.checkNotNull(plant, playersWhoHaveBoughtPlants,
                playersWhoHaveOptedOut);
        Integer highestBid = firstBidder.bidOnPlant(plant,
                plant.getMinimumPrice(), false);
        if (highestBid == null) {
            throw new IllegalStateException("First bid can't be a pass");
        }
        LOGGER.debug(firstBidder + " has put up the " + plant + " plant for "
                + highestBid);
        ...
        while (bidders.size() > 1) {
            ...
            if (bid == null) {
                ...
                throw new IllegalStateException("Must bid at least "
                        + minimumBid);
            }
            else {
                // It's a higher bid
    ...
    private void connectCities() {
         ...
         * remove any obolsete plants from the market after everyone has
         * finished building. We should actually be doing this after each new
         * connection is made, so that players can see the new state of the
         * market after each plant is removed (esp. knowing that Step 3 is about
         * to start), but that's a nicety we can add later.
         */
        int maxCityCount = 0;
        for (int i = players.size() - 1; i >= 0; i--) {
            ...
            final int totalCitiesConnected = board.getConnectedCities(player)
                    .size();
