...
public abstract class AbstractPlantMarket implements PlantMarket {
    ...
    // Constants
    protected static final int STEPS_ONE_AND_TWO_CURRENT_MARKET_SIZE = 4,
            INITIAL_MARKET_SIZE = STEPS_ONE_AND_TWO_CURRENT_MARKET_SIZE * 2;

    protected static final Logger LOGGER = LoggerFactory
            .getLogger(PlantMarket.class);
    ...
    public AbstractPlantMarket(final Set<Plant> startingPlants, final Deck deck) {
        ...
        if (startingPlants.size() != INITIAL_MARKET_SIZE) {
            throw new IllegalArgumentException("Invalid starting plants "
                    + startingPlants);
        }
        this.deck = deck;
    ...
    public void buyPlant(final Plant plant) {
        ...
        final Collection<Plant> buyablePlants = Arrays
                .asList(getCurrentMarket());
     ...
     * {@link Deck}. The market should automatically re-sort itself, because
     * it's implemented as a SortedSet.
     * 
    ...
    public void endTurn(final Step step) {
        if (!plants.isEmpty()) {
            if (Step.THREE.equals(step)) {
                ...
                // Put the highest plant in the market back under the deck for
                // step 3
                final Plant highestPlant = removePlant(plants.size() - 1,
                        Phase.FIVE);
