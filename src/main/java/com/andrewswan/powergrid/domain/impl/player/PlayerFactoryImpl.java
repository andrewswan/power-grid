...
public class PlayerFactoryImpl implements PlayerFactory {
     ...
     * @param inputDevice
     */
    public PlayerFactoryImpl(final InputDevice inputDevice) {
        this.inputDevice = inputDevice;
    }

    public Set<Player> getPlayers(final int numberOfPlayers, final Game game) {
        // Obtain a mutable list of all the possible player colours
        final List<Colour> unusedColours = new ArrayList<Colour>(
        ...
        for (int i = 0; i < numberOfPlayers; i++) {
            // Pass defensive copies of our collections to the overridable
            // methods

            // Get the new player's name
            final String name = getName(i, new ArrayList<String>(playerNames));
            ...
            final Colour colour = getColour(i, new ArrayList<Colour>(
                    unusedColours));
            ...
            final PlayerStrategy strategy = getStrategy(colour, game);
            if (strategy == null) {
                throw new IllegalStateException("No strategy for player " + i);
            }

            // Create the player and add them to our array
            players.add(createPlayer(numberOfPlayers, game, name, colour,
                    strategy));
        }
        return players;
    }
     ...
     */
    protected PlayerImpl createPlayer(final int numberOfPlayers,
            final Game game, final String name, final Colour colour,
            final PlayerStrategy strategy) {
        return new PlayerImpl(name, colour, numberOfPlayers, strategy, game);
    }

    /**
     * Returns the name of the player with the given player number (turn order
     * is randomised before the game starts, so it doesn't matter what order
     * players are in at this point). This implementation simply returns a name
     * like "Player 1". Subclasses can override this method to implement a more
     ...
     */
    protected String getName(final int playerNumber,
            final Collection<String> chosenNames) {
        if (playerNumber < 0) {
     ...
     */
    protected Colour getColour(final int playerNumber,
        ...
        if (unusedColours == null || unusedColours.isEmpty()) {
            throw new IllegalArgumentException("Invalid colours "
                    + unusedColours);
        }
        // Use the first one
     ...
     * returns the {@link DoNothingStrategy}. Subclasses can override this
     * method to provide sensible strategies for each player.
     * 
     ...
     */
    protected PlayerStrategy getStrategy(final Colour colour, final Game game) {
