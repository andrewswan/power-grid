...
public abstract class AbstractDeck implements Deck {
     ...
     * Constructor
     * 
     * @param plantsInDeck the plants that start the game in the deck (before
     *            any are removed based on the number of players); this array
     *            can't be empty
     ...
     */
    protected AbstractDeck(final Plant[] plantsInDeck,
            final int plantNumberOnTop, final int players) {
        ...
        if (plantOnTop == null) {
            throw new IllegalArgumentException("Plant number "
                    + plantNumberOnTop + " was not among those given");
        }
        ...
        for (int i = 0; i < plantsToRemove; i++) {
            try {
                ...
                throw new IllegalArgumentException("There must be at least "
                        + plantsToRemove + " plants in the deck for " + players
                        + " players");
            }
        }
