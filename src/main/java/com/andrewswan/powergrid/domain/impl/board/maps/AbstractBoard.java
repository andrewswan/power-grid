 ...
 */
public abstract class AbstractBoard extends UndirectedSparseGraph implements
        Board {
    // Properties
     ...
     * @param players the number of people playing; this determines how many
     *            areas of the board will be in play
     */
    protected AbstractBoard(final String name, final int players) {
        ...
        this.distanceCalculator = new DijkstraDistance(this,
                new LinkCostValue());
        this.name = name;
    ...
    protected final Area addArea(final String areaName) {
        ...
        if (!areas.add(area)) {
            throw new IllegalArgumentException("Already an area called '"
                    + areaName + "'");
        }
        return area;
    }
    ...
    /**
     * Adds a city with the given display and internal names to the given area
     * of the board
     * 
     * @param displayName can't be blank
     * @param internalName if blank, the display name is used as the internal
     *            name
     * @param area can't be <code>null</code>
     * @return the added City
     */
    protected final City addCity(final String displayName,
            final String internalName, final Area area) {
        Utils.checkNotNull(area);
        areas.add(area); // might already be there, doesn't matter
        final City city = (City) addVertex(new CityImpl(area, displayName,
                internalName));
        area.add(city);
        return city;
    }
    ...
    public void connectCity(final String cityName, final Player player,
            final Step step) {
    ...
    public int getTotalLinkCost(final City source, final City destination) {
        ...
        final Number linkCost = distanceCalculator.getDistance(source,
                destination);
        if (linkCost == null) {
            throw new IllegalStateException("No route from " + source + " to "
                    + destination);
        }
     ...
     * this method to choose them some other way. This implementation chooses
     * them randomly, ensuring that all selected areas are connected somehow.
     */
    protected void chooseAreasInPlay() {
        ...
        if (areas.size() < numberOfAreasInPlay) {
            throw new IllegalStateException("There are only " + areas.size()
                    + " areas on the board. " + numberOfAreasInPlay
                    + " are required for " + players + " players");
        }
        do {
    ...
    /**
     * Returns the number of areas of the board that will be in play for the
     * given number of players. Subclasses can override this method to vary the
     * standard Power Grid rule.
     * 
     * @param players
    ...
    public Integer getConnectionCost(final String cityName,
            final Player player, final Step step) {
        // Get the city to be connected
        ...
        // Add the cheapest link cost(s) from the player's existing cities (if
        // any)
        ...
        for (final City source : connectedCities) {
            ...
            if (linkCost == 0 || aLinkCost < linkCost) {
                // This link is the first found, or is cheaper than the cheapest
                // so far
                linkCost = aLinkCost;
            }
        }
