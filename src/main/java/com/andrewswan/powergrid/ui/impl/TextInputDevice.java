...
public class TextInputDevice implements InputDevice {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(TextInputDevice.class);
    ...
    private static final char COAL_CHARACTER = 'c', GARBAGE_CHARACTER = 'g',
            OIL_CHARACTER = 'o', URANIUM_CHARACTER = 'u';
     ...
     * Constructor
     * 
     * @param inputStream the stream from which to read the user's input; can't
     *            be <code>null</code>
     * @param outputDevice the device for sending output to the user, such as
     *            input prompts and error messages; can't be <code>null</code>.
     *            The caller is responsible for closing it.
     */
    public TextInputDevice(final InputStream inputStream,
    ...
    public Integer bidOnPlant(final Colour colour, final Plant plant,
            final int minimumBid, final boolean canPass) {
        String passOption = "";
        if (canPass) {
            passOption = ", pass = 0";
        }
        final String prompt = String.format(BID_PROMPT, colour,
    ...
    /**
     * Indicates whether the given bid is valid, based on the minimum allowed
     * bid and whether the player can pass.
     * 
     * @param bid
     * @param minimumBid
     * @param canPass
     * @return see above
     */
    private boolean isValidBid(final int bid, final int minimumBid,
    ...
    public ResourcePool getResourcesToBuy(final Colour colour) {
        ...
        for (final String token : StringUtils.split(readLine(), ' ')) {
            // The token should be a number followed by a character indicating
            // the
            // resource type
            try {
                ...
                outputDevice.showError(String.format(INVALID_RESOURCE_MESSAGE,
                        token));
     ...
     * @return see above
     * @throws InvalidResourceTokenException if the given token doesn't contain
     *             a valid quantity
     */
    private int getResourceQuantity(final String token)
            throws InvalidResourceTokenException {
        // Remove the resource indicator from the end of the token & convert to
        // int
        try {
    ...
    private Resource getResourceType(final String string)
        ...
        final char resourceIndicator = string.toLowerCase(LOCALE).charAt(
                string.length() - 1);
    ...
    public Integer selectPlantForAuction(final Colour colour,
            final Plant[] currentMarket, final boolean mandatory) {
        final Collection<Integer> validPlantNumbers = Utils
                .getPlantNumbers(currentMarket);
        ...
        final String prompt = String.format(SELECT_PLANT_PROMPT, colour,
                passOption);
     ...
     * @param mandatory whether the user is required to enter a valid plant
     *            number
     * @return <code>null</code> if they opt not to enter a plant number and
     *         mandatory is <code>false</code>
     */
    private Integer readPlantNumber(final String prompt,
            final Collection<Integer> validPlantNumbers, final boolean mandatory) {
     ...
     * @return see above
     */
    private boolean isValidPlantNumber(final int plantNumber,
            final Collection<Integer> validPlantNumbers, final boolean mandatory) {
     ...
     * implementation closes those streams. Subclasses can override this method
     * to provide alternative handling.
     * 
