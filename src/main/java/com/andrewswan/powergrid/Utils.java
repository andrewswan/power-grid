...
public final class Utils {
     ...
     * The first ones on the stack are: 0: java.lang.Thread 1:
     * com.andrewswan.powergrid.Utils 2: the class that called Utils
     */
    ...
    /**
     * Converts the given {@link Integer} to an int, treating <code>null</code>
     * as zero.
     * 
     * @param integer
     * @return see above
    ...
    public static void checkNotInCallStack(final Class<?> prohibitedClass) {
        ...
        for (int i = FIRST_INDIRECT_CALLER; i < stackTrace.length; i++) {
            ...
            try {
                ...
                LOGGER.debug("Checking caller " + i + " of type "
                        + callerClassName);
                ...
                // If the calling class isn't on the classpath, we can safely
                // ignore it
                LOGGER.debug("Ignoring caller " + i + " of type "
                        + callerClassName);
            }
        }
    }
     ...
     * numbers.
     * 
     * @param plants the array of plants to convert; can be <code>null</code>;
     *            any <code>null</code> elements are ignored
     * @return a non-<code>null</code> set in the same order
     */
    public static Set<Integer> getPlantNumbers(final Plant[] plants) {
        LOGGER.debug("Getting plant numbers for plant array "
                + Arrays.toString(plants));
        final Set<Integer> plantNumbers = new LinkedHashSet<Integer>(); // preserve
                                                                        // order
        if (plants != null) {
