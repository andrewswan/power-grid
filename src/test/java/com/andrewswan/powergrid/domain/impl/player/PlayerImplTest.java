...
public class PlayerImplTest {
    ...
    public void setUp() throws Exception {
        ...
        player = new PlayerImpl(PLAYER_NAME, Colour.BLACK, 3, mockStrategy,
                mockGame);
    }
    ...
    public void testConnectOneCity() {
        ...
        when(mockStrategy.getCitiesToConnect()).thenReturn(
                new String[] { cityName });
        final Board mockBoard = mock(Board.class);
        when(mockGame.getBoard()).thenReturn(mockBoard);
        when(mockGame.getStep()).thenReturn(STEP);
        when(mockBoard.getConnectionCost(cityName, player, STEP)).thenReturn(
                CONNECTION_COST);
